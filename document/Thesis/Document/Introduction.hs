...
thesisIntroduction =
    ...
    -- - Easyspec-evaluate to evaluate strategies
     do
        ...
        s
            "In order to write correct code, a programmer has to decide what it means for code to be correct, and then write code that matches these expectations."
        s
            "The process is further complicated by the fact that these two phases are rarely separated."
        lnbk
        s
            "The first aspect of writing correct code is to choose the appropriate language."
        s
            "Compilation, Static typing, immutability, functional programming, purity, higher order programming and laziness are just some examples of tools that make writing correct software easier."
        lnbk
        s
            "Next, there are the tools that are external to the chosen programming language."
        s
            "These tools include formal verification, model checking, various static analysis techniques, etc."
        s
            "They usually suffer from one of two problems: Either the programmer has to already have decided what it means for their code to be correct, or the method is so specific in its scope that it cannot help in general."
        question
            "I have no idea how to write it in such a way that you are happy with it. Any suggestions?"
        s
            "They are also usually too expensive in terms of engineering effort or compute power, or both, to be used in practice."
        lnbk
        ...
        question
            "I have a feeling that you won't be happy with this phrasing, any suggestions?"
        s
            "The idea of automated testing is that programmers could tell if their code was faulty before running the code in production."
        s
            "In principle, the programmer writes extra code that will automatically test whether the subject code is faulty."
        s
            "It seems that testing currently has the best balance between cost and benefit, but it is still too often beneficial from a business perspective to skip writing tests."
        lnbk
        l
            [ emph "Unit testing"
        ...
        s "In what follows, we will call this problem the coverage problem."
        lnbk
        l
        ...
        lnbk
        s
            "We contribute a new approach to taming the complexity of property discovery: signature inference."
        s
        ...
        l
            ...
            , dquoted "signature inference strategies"
            ]
        s
            "These represent a strategy to automatically supply input to a property discovery algorithm and combine the results."
        s
